...
import com.alimoradi.presentation.license.LicensesFragment
import com.alimoradi.presentation.thanks.SpecialThanksFragment
...
import com.alimoradi.sharedandroid.extensions.isIntentSafe
...
class NavigatorAboutImpl @Inject internal constructor(
    ...
    override fun joinBeta() {
        ...
        if (allowed()) {
            val intent = Intent(Intent.ACTION_VIEW)
            intent.data = Uri.parse("https://play.google.com/apps/testing/com.alimoradi.eliteplayer")
            if (activity.packageManager.isIntentSafe(intent)) {
                activity.startActivity(intent)
            } else {
                activity.toast(R.string.common_browser_not_found)
